...
using TypeInfo = ReSharperPlugin.Fathom.Models.TypeInfo;
...
public static class CppStructureWalker
{
        ...
        List<string> debugDiagnostics)
    {
        ...
        foreach (var kvp in classSpecifiers)
        {
            ...
            // Skip classes generated by DECLARE_* macros (e.g. DECLARE_DYNAMIC_MULTICAST_DELEGATE)
            if (IsMacroGeneratedClass(node)) continue;
        ...
        foreach (var kvp in declarators)
        {
            ...
            else if (containingClassNode != null)
            {
                // Member of a skipped class (macro-generated delegate, etc.) - ignore
                continue;
            }
            else
            {
    ...
    /// <summary>
    /// Detect classes generated by DECLARE_* macros (e.g. DECLARE_DYNAMIC_MULTICAST_DELEGATE).
    /// These expand into ClassSpecifier nodes but represent internal delegate plumbing, not
    /// user-authored types. Two detection strategies:
    ///   1. Walk up ancestors checking if any IS a MacroCall (class inside macro expansion).
    ///   2. Walk up ancestors checking PrevSibling at each level for a DECLARE_* MacroCall.
    /// </summary>
    private static bool IsMacroGeneratedClass(ITreeNode classSpecifierNode)
    {
        var current = classSpecifierNode;
        for (var depth = 0; depth < 8 && current != null; depth++)
        {
            var typeName = current.GetType().Name;
            if (typeName == "CppFile" || typeName == "TranslationUnit")
                break;

            // Check if this ancestor itself is a macro expansion
            if (typeName == "MacroCall")
            {
                var text = current.GetText();
                if (text != null && text.TrimStart().StartsWith("DECLARE_"))
                    return true;
            }

            // Check preceding siblings for a DECLARE_* MacroCall
            var sibling = current.PrevSibling;
            var steps = 0;
            while (sibling != null && steps < 5)
            {
                if (sibling.GetType().Name == "MacroCall")
                {
                    var text = sibling.GetText()?.Trim();
                    if (!string.IsNullOrEmpty(text))
                    {
                        if (text.StartsWith("DECLARE_"))
                            return true;
                        break; // Substantive non-DECLARE MacroCall, stop
                    }
                    // Ghost/expansion MacroCall (len=0, empty text) - skip it
                }
                else if (!string.IsNullOrWhiteSpace(sibling.GetText()))
                {
                    break;
                }
    ...
    private static string CleanTypeString(string raw)
    {
        ...
        while (true)
        {
            ...
            // Strip orphaned UE macro argument blocks: when the parser consumes the
            // macro name (UPROPERTY, UFUNCTION) as a separate token, only the
            // parenthesized arguments remain, e.g. "(BlueprintReadOnly, ...)\n Type*".
            // Strip a leading balanced (...) if followed by actual type text.
            if (!changed && result.StartsWith("("))
            {
                var depth = 0;
                for (var i = 0; i < result.Length; i++)
                {
                    ...
                    else if (result[i] == ')')
                    {
                        ...
                        if (depth == 0)
                        {
                            var remainder = result.Substring(i + 1).TrimStart();
    ...
    /// <summary>
    /// Extract UE macro annotations by navigating the PSI tree structure directly.
    ///
    /// In Rider's C++ PSI, UE macros produce two nodes:
    ///   1. A MacroCall sibling before the SimpleDeclaration, containing just the macro name
    ///      (e.g., "UCLASS", "UPROPERTY").
    ///   2. A CppUE4Metadata child within the Declaration/DeclarationSpecifiers, containing
    ///      the parsed arguments (e.g., "(BlueprintReadOnly, meta = (...))").
    ///
    /// This method finds both and combines them into a single annotation string.
    /// </summary>
    private static List<string> TryExtractUeAnnotations(ITreeNode node)
    {
        try
        {
            // Find the nearest SimpleDeclaration ancestor
            var simpleDecl = FindAncestorByTypeName(node, "SimpleDeclaration");
            if (simpleDecl == null) return null;

            // Find MacroCall preceding the SimpleDeclaration (contains macro name)
            var macroName = FindPrecedingUeMacroCall(simpleDecl);

            // Find CppUE4Metadata between node and SimpleDeclaration (contains parsed arguments)
            var metadataText = FindCppUe4Metadata(node, simpleDecl);

            // Combine macro name + metadata arguments
            if (macroName != null && metadataText != null)
                return new List<string> { macroName + metadataText };
            if (macroName != null)
                return new List<string> { macroName + "()" };
        }
        catch { }
    ...
    /// <summary>
    /// Walk preceding siblings of a node (skipping whitespace) to find a MacroCall
    /// whose text matches a known UE macro name.
    /// </summary>
    private static string FindPrecedingUeMacroCall(ITreeNode node)
    {
        ...
        while (sibling != null && steps < 5)
        {
            ...
            {
                var text = sibling.GetText()?.Trim();
                if (text != null && IsUeMacroName(text))
                    return text;
                break; // Non-UE macro (e.g. GENERATED_BODY), stop searching
            }

            // Only skip past whitespace/newline tokens
            if (!string.IsNullOrWhiteSpace(sibling.GetText()))
                break;
    ...
    /// <summary>
    /// Search for a CppUE4Metadata child node in ancestors between the given node
    /// and the SimpleDeclaration boundary.
    /// </summary>
    private static string FindCppUe4Metadata(ITreeNode node, ITreeNode boundary)
    {
        ...
        while (ancestor != null && ancestor != boundary)
        {
            ...
            {
                if (child.GetType().Name == "CppUE4Metadata")
    ...
    private static bool IsUeMacroName(string text)
    {
        foreach (var name in UeMacroNames)
        {
