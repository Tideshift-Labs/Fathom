...
namespace ReSharperPlugin.CoRider.Mcp
{
    /// <summary>
    /// Implements the MCP (Model Context Protocol) JSON-RPC handler.
    /// Translates MCP tool calls into internal HTTP requests to CoRider REST endpoints.
    /// No external SDK needed; the protocol is simple JSON-RPC 2.0 over HTTP.
    /// </summary>
    public class CoRiderMcpServer
    {
        private const string ProtocolVersion = "2025-03-26";
        private const string ServerName = "corider";
        ...
        public CoRiderMcpServer(int port)
        {
            _port = port;
        }
        ...
        private object HandleToolsCall(JsonElement paramsEl)
        {
            ...
            catch (Exception ex)
            {
                return new
                {
                    content = new[] { new { type = "text", text = "Error calling CoRider endpoint: " + ex.Message } },
                    isError = true
                };
            }
