# Technical Overview: CoRider Rider Plugin

This document provides deep technical details on the implementation, architecture, and design decisions of the CoRider Rider plugin.
...
Reads Blueprint audit JSON files generated by the **CoRider-UnrealEngine** companion plugin.
...
This plugin works with the companion [CoRider-UnrealEngine](https://github.com/kvirani/CoRider-UnrealEngine) UE editor plugin.
...
- **Audit output path**: `Saved/Audit/v<N>/Blueprints/...`. The `v<N>` version segment invalidates cached JSON automatically.
...
- **Port**: Defaults to 19876. Configurable via Settings > Tools > CoRider or `RIDER_INSPECTOR_PORT`.
